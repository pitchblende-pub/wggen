...
Peers=99 # 設定するクライアントの数。1〜9999の範囲。
ServerPort=51820 # サーバー側WireGuardが使用する実ポート
Endpoint=ddns.example.com:51820 # 外部から見た場合のサーバーアドレスとポート番号
EthernetInterface='' # サーバーから外部にアクセスするための実インターフェイス名（空欄の場合は自動取得を試行）
DNS='1.1.1.1, 2606:4700:4700::1111' # トンネル開通後にクライアントが参照するネームサーバー
                                    #（記載アドレスはCloudflare。元の環境のDNSを使用する場合はDNS=''と設定。）
#MTU=1394 #トンネル内でのMTU値（指定する場合）
